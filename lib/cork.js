...
class LoggingImplementation {
  constructor (findTag, resolveLogLevel, entryWriter, _options) {
    this._entryWriter = entryWriter
    this._findTag = findTag
    this._resolveLogLevel = resolveLogLevel
  ...
  spawn ({ name = this._name, threshold = this._threshold, decorations = {}, middleware = [] }) {
    return new LoggingImplementation(
      this._findTag,
      this._resolveLogLevel,
      this._entryWriter,
      {
        ...this._options,
        name,
...
class LoggingFacade {
  constructor (entryWriter, {
    ...
    this._logLevelLookup = logLevelsToLookup(logLevels)
    logLevels.forEach(([level, tag]) => {
      this[tag] = (message, ...meta) => {
        return this.log(level, message, ...meta)
      }
    })
    this._stack = [new LoggingImplementation(this._findTag.bind(this), this._resolveLogLevel.bind(this), entryWriter, { name, threshold, decorations, middleware })]
  }
